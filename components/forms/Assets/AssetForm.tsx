...
const AssetForm = () => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      eol_date: new Date(new Date().setFullYear(new Date().getFullYear() + 0)),
      purchase_cost: 0,
    },
  });
  ...
  useEffect(() => {
    if (watchWarranty >= 0) {
      const newEolDate = new Date(watchPurchaseDate);
      newEolDate.setFullYear(newEolDate.getFullYear() + watchWarranty);

      form.setValue("eol_date", newEolDate);
    }
  }, [watchWarranty, watchPurchaseDate, form]);
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsLoading(true);
    try {
      console.log("Form values before submission:", values);

      const response = await axios.post<ApiResponse>("/api/core/asset", {
      ...
      toast({
        title: "Error",
        description: axiosError.response?.data.message ?? "Failed to Add Asset",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    <Dialog>
      ...
      <DialogContent className="max-h-full max-w-[550px] p-0">
        <ScrollArea className="h-[90%] w-full p-6">
          <DialogHeader>
            <DialogTitle className="text-2xl">Create Asset</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Asset Name" type="text" {...field} />
                    </FormControl>
